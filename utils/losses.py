...
class LADELoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        pred_spread = (logit - torch.log(self.prior + 1e-9) + torch.log(self.balanced_prior + 1e-9)).T
...
class SymmetricKLLoss(nn.Module):
    ...
    def __init__(self, reduction='batchmean', temperature=1.0, alpha=1.0):
        super().__init__()
    ...
    def forward(self, logits_p, logits_q):
        # 应用温度缩放
        logits_p = logits_p / self.temperature
        logits_q = logits_q / self.temperature
        ...
        # 计算对称KL散度: 0.5 * (KL(p||q) + KL(q||p))
        kl_pq = F.kl_div(log_q, p, reduction='none', log_target=False)
        kl_qp = F.kl_div(log_p, q, reduction='none', log_target=False)
        symmetric_kl = 0.5 * (kl_pq + kl_qp)

        # 应用聚合方式
        if self.reduction == 'batchmean':
            batch_size = logits_p.shape[0]
            loss = symmetric_kl.sum() / batch_size
        elif self.reduction == 'mean':
            loss = symmetric_kl.mean()
        elif self.reduction == 'sum':
            loss = symmetric_kl.sum()

        return loss * self.alpha


class ComplexFusionLoss(nn.Module):
    """增强的复合融合损失函数，集成多种粗类细类相关损失"""
    ...
    def __init__(
        ...
        # 新增的粗类细类相关损失权重
        self.lambda_symmetric_kl = lambda_symmetric_kl

        # 初始化损失函数组件
        self.LA = LogitAdjustedLoss(cls_num_list=cls_num_list)
        self.Focal = FocalLoss(gamma=focal_gamma)
        self.ce_loss = nn.CrossEntropyLoss()
        self.SymmetricKL = SymmetricKLLoss(temperature=temperature)

    def forward(self, output, fine_targets):
        total_loss = 0.0
        loss_components = {}

        # === 基础损失函数 ===
        # 1. LogitAdjustedLoss for logits_standard
        # if 'logits_global' in output:
        #     # loss_standard = self.LA(output['logits_global'], fine_targets)
        #     loss_standard = self.ce_loss(output['logits_global'], fine_targets)
        #     total_loss += self.lambda_standard * loss_standard
        #     loss_components['logits_global'] = loss_standard.item()
        #
        # # 2. CrossEntropyLoss for logits_similar (coarse classification)
        # if 'logits_patch' in output:
        #     # loss_similar = self.LA(output['logits_patch'], fine_targets)
        #     loss_similar = self.ce_loss(output['logits_patch'], fine_targets)
        #     total_loss += self.lambda_patch * loss_similar
        #     loss_components['logits_patch'] = loss_similar.item()

        # 3. LogitAdjustedLoss for total_logits
        if 'logits' in output:
            loss_total = self.LA(output['logits'], fine_targets)
            # loss_total = self.ce_loss(output['logits'], fine_targets)
            total_loss += self.lambda_total * loss_total
