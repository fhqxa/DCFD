...
class Evaluator:
    ...
    def process(self, mo, gt):
        ...
        # 保存原始模型输出用于后续分析
        if not hasattr(self, '_mo_outputs'):
            self._mo_outputs = []
        self._mo_outputs.extend(mo.data.cpu().numpy())
    ...
    def evaluate(self, is_per_class =  False):
        results = OrderedDict()
        ...
        print(
            f"* worst_case_acc: {worst_case_acc:.4f}%\n"
            # f"* lowest_recall: {lowest_recall:.4f}%\n"
            f"* hmean_acc: {hmean_acc:.4f}%\n"
            f"* gmean_acc: {gmean_acc:.4f}%"
        )
        ...
        if self.many_idxs is not None and self.med_idxs is not None and self.few_idxs is not None:
            ...
            print(f"* many: {many_acc:.4f}%  med: {med_acc:.4f}%  few: {few_acc:.4f}%")
    ...
    def _analyze_per_class_performance(self):
        ...
        print("\n=> 每个类别的详细性能分析:")
        print("类别ID\t准确率\t最常被误分类为(前5)")

        for i, label in enumerate(unique_labels):
            # 计算该类别的准确率
            tp = cm[i, i]  # 真正例
            total = cm[i, :].sum()  # 该类别的总样本数
            accuracy = 100.0 * tp / total if total > 0 else 0

            # 找出最容易被错误分类到的类别（排除正确分类）
            misclassified_counts = cm[i, :].copy()
            misclassified_counts[i] = 0  # 排除正确分类的情况

            # 获取前5个最常见的错误分类
            top_5_errors_idx = np.argsort(misclassified_counts)[::-1][:5]
            top_5_errors = [(unique_labels[idx], misclassified_counts[idx])
                            for idx in top_5_errors_idx if misclassified_counts[idx] > 0]

            # 打印结果
            error_str = "\t".join([f"{cls}(#{count})" for cls, count in top_5_errors])
            print(f"{label}\t{accuracy:.4f}%\t{error_str}")
