...
from utils.config import _C as base_cfg, auto_update_config
from utils.logger import setup_logger

from trainer import Trainer


def load_auto_config(args):
    cfg_data_file = os.path.join("./configs/data", args.data + ".yaml")
    cfg_model_file = os.path.join("./configs/model", args.model + ".yaml")

    cfg = base_cfg.clone()
    auto_update_config(cfg, cfg_data_file)
    auto_update_config(cfg, cfg_model_file)
...
def main(args):
    ...
    if cfg.visualization_only == True:
        if cfg.model_dir is None:
            cfg.model_dir = cfg.output_dir[:cfg.output_dir.index("_visualization_only_True")]
            print("Model directory: {}".format(cfg.model_dir))
        trainer.load_model(cfg.model_dir)
